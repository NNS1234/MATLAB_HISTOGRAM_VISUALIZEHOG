...
figure, imshow(n_direction_dis); title('SOBEL gradient direction');

% Displaying Filtered Image 
filter_image = uint8(filter_image); 
figure, imshow(filter_image); title('Sobel filter'); 
  
% Define a threshold value 
thresholdValue = 72; % varies between [0 255] 
output_image = max(filter_image, thresholdValue); 
output_image(output_image == round(thresholdValue)) = 0; 
  
% Displaying Output Image 
output_image = imbinarize(output_image); 
figure, imshow(output_image); title('After threshold Sobel edge detection'); 